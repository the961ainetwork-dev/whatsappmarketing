...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      if (s) s.access_token = s.access_token ? '••••' + String(s.access_token).slice(-6) : '';
    ...
    if (req.method === 'PUT') {
      ...
      for (const k of ['phone_number_id', 'waba_id', 'business_name', 'ai_prompt', 'owner_phone']) if (k in b) patch[k] = String(b[k] || '').slice(0, 4000);
    ...
    if (req.method === 'POST') {
      ...
      const s = await getSettings(user.id);
      if (!s?.phone_number_id || !s?.access_token) return res.status(400).json({ error: 'Add your WhatsApp credentials first' });

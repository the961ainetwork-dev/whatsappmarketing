import { sb, readBody, cors, hashPass, newSalt, newToken } from './_lib.js';
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (b.action === 'create') {
        ...
        if (!email.includes('@') || password.length < 6) return res.status(400).json({ error: 'Valid email + password (6+) required' });
        const plan = ['free', 'starter', 'pro', 'agency'].includes(b.plan) ? b.plan : 'free';
        const salt = newSalt(), token = newToken();
        const r = await sb('wm_users', { method: 'POST', body: JSON.stringify([{ email, salt, pass_hash: hashPass(password, salt), token, plan, status: 'active' }]) });
        if (!r.ok) { const t = await r.text(); return res.status(400).json({ error: t.includes('duplicate') ? 'Email already exists' : 'Create failed' }); }
        const [u] = await r.json();
        await sb('wm_settings', { method: 'POST', body: JSON.stringify([{ user_id: u.id }]) });
        return res.status(200).json({ ok: true, id: u.id, email });
      }
